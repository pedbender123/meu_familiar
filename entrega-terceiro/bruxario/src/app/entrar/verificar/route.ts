...
export async function GET(req: NextRequest) {
  ...
  /**
   * Só existe acesso de admin nesta versão.
   *
   * Não há conta de cliente: quem compra recebe o PDF por e-mail e o link da
   * revelação, e não entra em lugar nenhum. Um token que não seja do
   * administrador não abre porta alguma — em vez de virar "conta", vira nada.
   */
  if (validado.tipo !== 'admin' || !ehAdmin(validado.email)) {
    return NextResponse.redirect(new URL('/painel/entrar?erro=1', req.url));
  }
  const tipo = 'admin' as const;


  const { token: sessao, expiraEm } = abrirSessao(validado.email, tipo);
  registrarEvento('admin_entrou');
