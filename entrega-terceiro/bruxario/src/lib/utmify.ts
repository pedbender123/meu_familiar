...
export async function reportarPedido(pedido: PedidoParaUtmify): Promise<boolean> {
  ...
  const corpo = {
    ...
    platform: process.env.UTMIFY_PLATAFORMA ?? 'DirectPag',
 ...
 *
 * O padrão é `pix` e não um erro: um método desconhecido não pode impedir a
 * venda de aparecer no relatório — melhor uma linha com o método errado que
 * uma venda invisível.
 */
