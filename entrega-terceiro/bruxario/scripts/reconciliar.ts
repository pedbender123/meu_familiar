...
import { pagamentoEhFake } from '../src/nucleo/checkouts/directpag';
...
async function main() {
  if (pagamentoEhFake()) {
    console.log('Sem DIRECTPAG_API_TOKEN configurado — nada para reconciliar.');
    return;
  }

  const ate = new Date();
  const desde = new Date(ate.getTime() - horas * 3_600_000);
  console.log(`Reconciliando ${desde.toISOString()} → ${ate.toISOString()}`);
